...
func (a *Account) SignIn(writer http.ResponseWriter, req *http.Request) {
	switch req.Method{
	case http.MethodGet:a.signInGet(writer, req)
	case http.MethodPost: a.signInPost(writer, req)
	default: http.Error(writer, http.StatusText(http.StatusMethodNotAllowed), http.StatusMethodNotAllowed)
	}
}
